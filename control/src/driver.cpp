...
void MotorDriver::initialMotors()
{
    ...
    analogWrite(enable_pin_1, default_speed + this->increase);
    analogWrite(enable_pin_2, default_speed + this->increase);
    // stop the motors
    MotorDriver::stop();
}
...
void MotorDriver::incraeseSpeed()
{
    int current = this->getCurrentSpeed();
    Serial.println("Increasing, current speed:");
    Serial.println(current);
    if (current < max_speed)
    {
        this->setSpeed(current + 5);
    }
}
...
{
    int current = this->getCurrentSpeed();
    Serial.println("Decreasing, current speed:");
    Serial.println(current);
