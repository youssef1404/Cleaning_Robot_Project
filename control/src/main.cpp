...
// PID Objects with dt and alpha
PIDController PID[]{PIDController(kp0, ki0, kd0, OUTPUTLIMITS, DEADZONE, 0.01, 0.8),
                    PIDController(kp1, ki1, kd1, OUTPUTLIMITS, DEADZONE, 0.01, 0.8)};
...
void speed_controll()
{
  ...
  pid_output[0] = constrain(PID[0].calculateOutput(speed_feedback[0]), -OUTPUTLIMITS, OUTPUTLIMITS);
  pid_output[1] = constrain(PID[1].calculateOutput(speed_feedback[1]), -OUTPUTLIMITS, OUTPUTLIMITS);
